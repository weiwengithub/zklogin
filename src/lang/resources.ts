export const resources = {
  en: {
    translation: {
      ...
      '66f6b490': 'Generate user Oct address',
      ...
      '4a7add7c': 'Randomness',
      '3a96f638': 'Fetch current Epoch (via SuiClient)',
      ...
      ec71ef53: 'User Salt is used to eliminate the one-to-one correspondence between the OAuth identifier (sub) and the on-chain Oct address, avoiding linking Web2 credentials with Web3 credentials.',
      ...
      '2fb333f5': "Step 5: Generate User's Oct Address",
      e05797f4: "The user's Oct address is determined by <1>sub</1>,<1>iss</1>,<1>aud</1> and <1>user_salt</1> together. For the same JWT, <1>sub</1>, <1>iss</1>, and <1>aud</1> will not change with each login.",
      c9bbf457: 'Generate Oct Address',
      ...
      '16ebd660': 'Use the generated extended ephemeral public key (extendedEphemeralPublicKey) to generate ZK Proof. OCT provides a backend service (or you can run a Docker).',
      ...
      '6591b962': 'Before executing the transaction, please recharge zkLogin with a small amount of OCT as the gas fee for initiating the transaction.'
    },
  },
  zh: {
    translation: {
      ...
      '62a0a307': '临时秘钥对用来对<1>Transaction</1>进行签名',
      '9ec629a8': '储存在浏览器会话中',
      '4f04f1f8': '第二步：获取JWT',
      '56adebff': '所需参数：',
      ...
      bf54d75b: '临时秘钥对的有效期',
      '4a7add7c': '随机种子',
      '3a96f638': '获取当前Epoch (通过SuiClient)',
      '6d47d563': '当前Epoch:',
      ...
      ec71ef53: '用户 Salt 用于消除 OAuth 标识符 （sub） 与链上 Oct 地址的一一对应关系，以避免将 Web2 凭证与 Web3 凭证链接。',
      cb63dedd: '因此必须保管好 Salt，丢失后用户则无法找回当前 Salt 生成的地址。',
      c4a666f0: '<1>保存在哪：</1>\n' +
        '      <1>1.要求用户记住(发送到用户邮箱)</1>\n' +
        '      <1>2.储存在客户端(浏览器)</1>\n' +
        '      <1>3.保存在APP Backend数据库，与UID一一对应</1>',
      '2fb333f5': '第五步：获取用户的 Oct 地址',
      e05797f4: '用户 Oct 地址由 <1>sub</1> 、 <1>iss</1> 、\n' +
      ...
      '71c429d2': '生成扩展的临时公钥',
      '16ebd660': '使用生成临时的公钥扩展(extendedEphemeralPublicKey)来生成ZK Proof，OCT 提供了一个 backend service（也可以 Run 一个 Docker）',
      '33893c96': '获取ZK Proof',
      acf1b947: '第七步：组装 zkLogin 签名并提交交易',
      d58c9e1e: '每个 ZK 证明都与一个临时密钥对相关联。储存在适当位置，可以重复作为证明用来签署任意数量的交易，直到临时密钥对过期',
      '6591b962': '执行交易前，请给 zkLogin 充值少量的 OCT 作为发起交易的 gas fee'
    },
  },
};
